defmodule Tracms.AccountsTest do
  use Tracms.DataCase
  ...
  alias Tracms.Accounts.{User, UserToken}
  ...
  describe "roles and profile metadata" do
    test "list_roles/0 returns stored roles" do
      role =
        role_fixture(%{key: "regional_admin", name: "Regional Administrator", scope: :region})

      assert Enum.any?(Accounts.list_roles(), &(&1.id == role.id))
    end

    test "update_user_profile/2 assigns role and office" do
      user = user_fixture()
      role = role_fixture()
      ...
      assert updated_user.employee_number == "EMP-001"
      assert updated_user.status == :active
      assert updated_user.role.id == role.id
      assert updated_user.office.id == office.id
      assert updated_user.office.division.id == office.division.id
    end

    test "approve_user/1 marks a user as active" do
      user = user_fixture()

      assert {:ok, approved_user} = Accounts.approve_user(user)
  ...
  describe "get_user_by_session_token/1" do
    ...
    test "preloads role and office data when assigned", %{user: user} do
      role = role_fixture()
      office = office_fixture()
      ...
      assert session_user.role.key == role.key
      assert session_user.office.code == office.code
      assert session_user.office.division.id == office.division.id
    end
