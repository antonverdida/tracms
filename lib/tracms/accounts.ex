defmodule Tracms.Accounts do
  ...
  alias Tracms.Accounts.{Role, User, UserNotifier, UserToken}

  @user_preloads [:role, office: [:division]]
  ...
  @doc """
  Returns all available roles for user assignment and authorization checks.
  """
  def list_roles do
    Role
    |> order_by([role], asc: role.scope, asc: role.name)
    |> Repo.all()
  end

  @doc """
  Gets a role by its stable key.
  """
  def get_role_by_key(key) when is_binary(key) do
    Repo.get_by(Role, key: key)
  end

  @doc """
  Returns a profile changeset for user metadata managed after registration.
  """
  ...
  @doc """
  Updates the user profile and access metadata.
  """
  ...
  See `Tracms.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Tracms.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  defp preload_user_context(user), do: Repo.preload(user, @user_preloads)

  defp preload_user_context_result({:ok, user}), do: {:ok, preload_user_context(user)}
  defp preload_user_context_result(other), do: other
end
