defmodule TracmsWeb.CoreComponents do
  @moduledoc """
    ...
    >
      <div class={[
        ...
        <.icon
          ...
          class="mt-0.5 size-5 shrink-0 text-[var(--tracms-primary)]"
        />
        <.icon
          :if={@kind == :error}
      ...
      <label class="inline-flex items-center gap-3 text-sm font-medium">
        ...
        <span class="label">
          <input
            ...
            class={
              @class ||
                "size-4 rounded border border-[var(--tracms-border)] text-[var(--tracms-primary)]"
            }
            {@rest}
          />
          <span>{@label}</span>
    ...
    <div class="field-wrap">
      <label>
        ...
        <select
          ...
          class={[
            @class || "field-select",
            @errors != [] && (@error_class || "field-error")
          ]}
          multiple={@multiple}
          {@rest}
        >
  ...
  defp error(assigns) do
    ~H"""
    <p class="mt-1 flex items-center gap-2 text-sm text-[var(--tracms-danger)]">
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", "pb-4"]}>
      <div>
        ...
        <p :if={@subtitle != []} class="text-sm text-[var(--tracms-text-muted)]">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(TracmsWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(TracmsWeb.Gettext, "errors", msg, opts)
    end
  end
